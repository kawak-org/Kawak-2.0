...
interface Props {
  tags: string[];
  handleDataTags: (data: any) => void;
}

const TagInput = ({ tags, handleDataTags }: Props) => {
  ...
  const handleTags = (e) => {
    ...
    handleDataTags([...tags, value]);
    e.target.value = "";
  };
  const removeTag = (index: any) => {
    handleDataTags(tags.filter((el, i) => i !== index));
  };
      ...
      <div className="flex flex-wrap gap-[1rem] mx-2">
        {items.map((item, index) => (
          <button
            ...
            onClick={() => {
              handleDataTags(tags.concat(item.text));
